(function ($) {
    ...
    var get_url       = function (endpoint) {
            return sadesmarket_params.sadesmarket_ajax_url.toString().replace(
                '%%endpoint%%',
                endpoint
            );
        },
    ...
    $(document).on('click', '.ovic-tabs .tabs-list .tab-link, .ovic-accordion .panel-heading a', function (e) {
        ...
        if ($data.ajax == 1 && !$this.hasClass('loaded')) {
            ...
            $.ajax({
                ...
                data     : {
                    security: sadesmarket_params.security,
                ...
                success  : function (response) {
                    ...
                    if (response) {
                        ...
                        if ($tabID.find('.owl-slick').length) {
                            $tabID.find('.owl-slick').sadesmarket_init_carousel();
                        }
                        if ($tabID.find('.equal-container.better-height').length) {
                            $tabID.find('.equal-container.better-height').sadesmarket_better_equal_elems();
                        }
                        if ($tabID.find('.sadesmarket-countdown').length && $.fn.sadesmarket_countdown) {
                            $tabID.find('.sadesmarket-countdown').sadesmarket_countdown();
                        }
                        if ($tabID.find('.ovic-products').length && $.fn.sadesmarket_load_infinite) {
                            $tabID.find('.ovic-products').sadesmarket_load_infinite();
                        }
                        if ($tabID.find('.yith-wcqv-button,.compare-button a.compare,.entry-summary a.compare,.yith-wcwl-add-to-wishlist a').length) {
                            $tabID.find('.yith-wcqv-button,.compare-button a.compare,.entry-summary a.compare,.yith-wcwl-add-to-wishlist a').sadesmarket_bootstrap_tooltip();
                        }
                    } else {
                        $tabID.html('<div class="alert alert-warning">' + sadesmarket_params.tab_warning + '</div>');
                    }
                    /* for accordion */
    ...
    $.fn.sadesmarket_init_carousel      = function () {
    ...
    $.fn.sadesmarket_better_equal_elems = function () {
        if (!Mobile_Detect.Mobile() && sadesmarket_params.disable_equal == false) {
            var $this = $(this);

            $this.on('sadesmarket_better_equal_elems', function () {
                setTimeout(function () {
                    $this.each(function () {
            ...
            }).trigger('sadesmarket_better_equal_elems');

            $(window).on('resize', function () {
                $this.trigger('sadesmarket_better_equal_elems');
            });
        }
    };
    $.fn.sadesmarket_sticky_header      = function () {
        $(this).each(function () {
            var $this   = $(this),
                $sticky = $this.find('.header-sticky'),
                $height = $sticky.height();

            if (sadesmarket_params.sticky_menu == 'template') {
            ...
            $(document).on('scroll', function (event) {
                ...
                if (st > sh) {
                    ...
                    $('#header-sticky').find('.sadesmarket-dropdown.open').removeClass('open');
                }
            });
        });
    };
    ...
    $(document).on('click', function (event) {
        var $target  = $(event.target).closest('.sadesmarket-dropdown'),
            $current = $target.closest('.sadesmarket-parent-toggle'),
            $parent  = $('.sadesmarket-dropdown');

        if ($target.length) {
            $parent.not($target).not($current).removeClass('open');
            if ($(event.target).is('[data-sadesmarket="sadesmarket-dropdown"]') ||
                $(event.target).closest('[data-sadesmarket="sadesmarket-dropdown"]').length) {
            ...
            $('.sadesmarket-dropdown').removeClass('open');
    ...
    /* BUTTON TOOLTIP */
    $.fn.sadesmarket_bootstrap_tooltip = function () {
    ...
    /* ZOOM IMAGE */
    $.fn.sadesmarket_zoom_product = function () {
    ...
    $(document).on('added_to_wishlist removed_from_wishlist', function () {
        $.ajax({
            type   : 'GET',
            url    : get_url('update_wishlist_count'),
            data   : {
                security: sadesmarket_params.security
            },
    ...
    if (sadesmarket_params.ajax_comment == 1) {
    ...
    $(document).on('click', '.more_seller_product_tab > a', function () {
        ...
        if ($(id).find('ul.products').length) {
            $(id).find('ul.products').sadesmarket_better_equal_elems();
        }
    });

    $(document).on('change', '#sadesmarket_disabled_popup_by_user', function () {
        if ($(this).is(":checked")) {
            set_cookie('sadesmarket_disabled_popup_by_user', 'true');
        } else {
            set_cookie('sadesmarket_disabled_popup_by_user', '');
        }
    });
    ...
    $(document).on('wc-product-gallery-after-init', function (event, gallery, params) {
        if ($(this).find('.flex-control-thumbs').length) {
            $(this).find('.flex-control-thumbs').sadesmarket_init_carousel();
        }
    });

    $(document).on('ovic_success_load_more_post', function (event, content) {
        if ($(event.target).find('.add-to-cart>a,.yith-wcqv-button,.compare-button a.compare,.entry-summary a.compare,.yith-wcwl-add-to-wishlist a').length) {
            $(event.target).find('.add-to-cart>a.yith-wcqv-button,.compare-button a.compare,.entry-summary a.compare,.yith-wcwl-add-to-wishlist a').sadesmarket_bootstrap_tooltip();
        }
        if ($(event.target).find('.owl-slick').length) {
            $(event.target).find('.owl-slick').sadesmarket_init_carousel();
        }
        if ($('.equal-container.better-height').length) {
            $('.equal-container.better-height').sadesmarket_better_equal_elems();
        }
    });
    ...
    $(document).on('updated_wc_div', function (event) {
        if ($(event.target).find('.cross-sells .owl-slick').length > 0) {
            $(event.target).find('.cross-sells .owl-slick').sadesmarket_init_carousel();
        }
    });
    ...
    window.addEventListener("load", function load() {
         ...
         * */
        if ($('.owl-slick').length) {
            $('.owl-slick').sadesmarket_init_carousel();
        }
        if ($('.elementor-section-slide').length) {
            $('.elementor-section-slide').sadesmarket_init_carousel();
        }
        if ($('.equal-container.better-height').length) {
            $('.equal-container.better-height').sadesmarket_better_equal_elems();
        }
         ...
         * popup newsletter
         * */
        if ($('.sadesmarket-popup-newsletter').length && get_cookie('sadesmarket_disabled_popup_by_user') !== 'true' && $.fn.magnificPopup) {
            var popup  = document.getElementById('sadesmarket-popup-newsletter'),
            ...
            setTimeout(function () {
                $.magnificPopup.open({
                    items       : {
                        src: '#sadesmarket-popup-newsletter'
                    },
                    type        : 'inline',
                    removalDelay: 600,
                    callbacks   : {
        ...
        if (!Mobile_Detect.any()) {
            if ($('.product-page-grid .site-main > .product').length) {
                $('.product-page-grid .site-main > .product').sadesmarket_zoom_product();
            }
            if ($('.product-page-slide .site-main > .product').length) {
                $('.product-page-slide .site-main > .product').sadesmarket_zoom_product();
            }
            if ($('.product-page-sticky .site-main > .product').length) {
                $('.product-page-sticky .site-main > .product').sadesmarket_zoom_product();
            }
            if ($('.header').length && sadesmarket_params.sticky_menu !== 'none' && window.matchMedia("(min-width: 1199px)").matches) {
                $('.header').sadesmarket_sticky_header();
            }
            if ($('.yith-wcqv-button,.compare-button a.compare,.entry-summary a.compare,.yith-wcwl-add-to-wishlist a').length) {
                $('.yith-wcqv-button,.compare-button a.compare,.entry-summary a.compare,.yith-wcwl-add-to-wishlist a').sadesmarket_bootstrap_tooltip();
            }
        }
        /* SCROLLBAR */
        if ($.fn.scrollbar) {
    ...
    if (sadesmarket_params.is_preview) {
        //
        // Elementor scripts
        //
        $(window).on('elementor/frontend/init', function () {
            elementorFrontend.hooks.addAction('frontend/element_ready/global', function ($scope, $) {
                $scope.find('.owl-slick').sadesmarket_init_carousel();
                $scope.find('.elementor-section-slide').sadesmarket_init_carousel();
                $scope.find('.equal-container.better-height').sadesmarket_better_equal_elems();
                if ($.fn.sadesmarket_countdown) {
                    $scope.find('.sadesmarket-countdown').sadesmarket_countdown();
                }
            });
        });
    }
