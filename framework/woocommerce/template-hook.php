 ...
 */
add_action( 'woocommerce_product_query', 'sadesmarket_product_query' );
/**
 *
 * REMOVE CSS
 */
...
add_filter( 'woocommerce_comment_pagination_args',
    function ( $args ) {
        $args['prev_text'] = esc_html__( 'Prev', 'sadesmarket' );
        $args['next_text'] = esc_html__( 'Next', 'sadesmarket' );
...
add_filter( 'woocommerce_catalog_orderby',
    function ( $options ) {
        $options['menu_order'] = esc_html__( 'Default Sorting', 'sadesmarket' );
        $options['popularity'] = esc_html__( 'Popularity', 'sadesmarket' );
        $options['rating']     = esc_html__( 'Average Rating', 'sadesmarket' );
        $options['date']       = esc_html__( 'Latest', 'sadesmarket' );
        $options['price']      = esc_html__( 'Price: Low To High', 'sadesmarket' );
        $options['price-desc'] = esc_html__( 'Price: High To Low', 'sadesmarket' );
        $options['sale']       = esc_html__( 'Sale', 'sadesmarket' );
        $options['on-sale']    = esc_html__( 'On-Sale', 'sadesmarket' );
        $options['feature']    = esc_html__( 'Feature', 'sadesmarket' );
 ...
 */
add_filter( 'loop_shop_per_page', 'sadesmarket_loop_shop_per_page', 20 );
add_filter( 'woof_products_query', 'sadesmarket_woof_products_query', 20 );
/**
 *
 * CUSTOM SHOP CONTROL
 */
...
add_action( 'woocommerce_before_shop_loop_item_title', 'sadesmarket_template_loop_product_thumbnail', 10 );
/**
 *
 * CUSTOM PRODUCT NAME
 */
remove_action( 'woocommerce_shop_loop_item_title', 'woocommerce_template_loop_product_title', 10 );
add_action( 'woocommerce_shop_loop_item_title', 'sadesmarket_template_loop_product_title', 10 );
/**
 *
...
add_filter( 'woocommerce_output_related_products_args',
    function ( $args ) {
        $args['posts_per_page'] = sadesmarket_get_option( 'woo_related_perpage', '6' );
 ...
 * HOOK MINI CART
 */
add_filter( 'woocommerce_add_to_cart_fragments', 'sadesmarket_cart_link_fragment' );
/**
 *
 * HOOK MY ACCOUNT
 */
...
add_filter( 'woocommerce_get_image_size_gallery_thumbnail',
    function () {
        $size = apply_filters( 'sadesmarket_get_image_size_gallery_thumbnail', $size = array(
            'width'  => 58,
            'height' => 0,
            'crop'   => 0,
        ) );
...
if ( class_exists( 'WeDevs_Dokan' ) ) {
    add_action( 'woocommerce_shop_loop_item_title', 'sadesmarket_dokan_sold_by_text', 10 );
    add_action( 'woocommerce_shop_loop_item_title', function () {
        echo '<div></div>';
    }, 10 );
    add_action( 'woocommerce_single_product_summary', 'sadesmarket_dokan_sold_by_text', 6 );
    if ( !function_exists( 'sadesmarket_dokan_sold_by_text' ) ) {
        function sadesmarket_dokan_sold_by_text()
        {
            global $product;
            if ( $product ) {
                ...
                echo apply_filters( 'ovic_dokan_sold_by_text',
                    sprintf( '<a class="by-vendor-name-link" href="%s"><span class="text">%s</span> %s</a>',
                        ...
                        esc_html__( 'Sold by', 'sadesmarket' ),
...
if ( class_exists( 'WCMp' ) ) {
    ...
    if ( !function_exists( 'sadesmarket_wcmp_sold_by_text' ) ) {
        function sadesmarket_wcmp_sold_by_text()
        {
            global $WCMp;
    ...
    add_action( 'init', 'sadesmarket_wcmp_sold_by_text' );
}
/**
 *
 * QUANTITY ARROWS
