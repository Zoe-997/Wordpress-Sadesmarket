 ...
 * GET POST TYPE
 **/
if ( !function_exists( 'sadesmarket_get_post_type' ) ) {
    function sadesmarket_get_post_type()
    {
 ...
 * EFFECT STYLE
 **/
if ( !function_exists( 'sadesmarket_nav_style' ) ) {
    function sadesmarket_effect_style( $unset = array() )
    {
        $effect = array(
            ...
            'effect effect faded-in'        => esc_html__( 'Faded In', 'sadesmarket' ),
            'effect bounce-in'              => esc_html__( 'Bounce In', 'sadesmarket' ),
            'effect gray-filter'            => esc_html__( 'Gray Filter', 'sadesmarket' ),
 ...
 **/
if ( !function_exists( 'sadesmarket_nav_style' ) ) {
    function sadesmarket_nav_style()
    {
        return array(
            '' => esc_html__( 'Style 01', 'sadesmarket' ),
        );
    }
}
/**
 ...
 **/
if ( !function_exists( 'sadesmarket_is_mobile' ) ) {
    function sadesmarket_is_mobile()
    {
        $mobile        = false;
        $mobile_enable = sadesmarket_get_option( 'mobile_enable' );
 ...
 * GET OPTION
 **/
if ( !function_exists( 'sadesmarket_get_option' ) ) {
    function sadesmarket_get_option( $option_name = '', $default = '' )
    {
        $cs_option = array();
        ...
        $options = apply_filters( 'sadesmarket_get_customize_option', $cs_option, $option_name, $default );
...
if ( !function_exists( 'sadesmarket_preview_options' ) ) {
    function sadesmarket_preview_options( $name )
    {
        $cache_key       = '';
        ...
        if ( sadesmarket_get_option( 'enable_cache_option' ) ) {
            $cache_key = sanitize_key( implode( '-', array( 'sadesmarket-preview', $name, SADESMARKET ) ) );
...
if ( !function_exists( 'sadesmarket_get_header' ) ) {
    function sadesmarket_get_header()
    {
        $header_options = sadesmarket_theme_option_meta(
...
if ( !function_exists( 'sadesmarket_get_footer' ) ) {
    function sadesmarket_get_footer()
    {
        $footer_option = sadesmarket_theme_option_meta(
            '_custom_metabox_theme_options',
            'footer_template',
            'metabox_footer_template',
            'style-01'
        );
        $mobile_footer = sadesmarket_get_option( 'mobile_footer', 'inherit' );
        if ( sadesmarket_is_mobile() && $mobile_footer != 'inherit' ) {
            $footer_option = sadesmarket_get_option( 'mobile_footer', 'inherit' );
        }
 ...
 */
if ( !function_exists( 'sadesmarket_edit_link' ) ) {
    function sadesmarket_edit_link( $id = 0, $text = null )
    {
        if ( !$post = get_post( $id ) ) {
            return;
        }
        ...
        if ( null === $text ) {
            $text = esc_html__( 'Edit This', 'sadesmarket' );
        }
        /**
...
if ( !function_exists( 'sadesmarket_theme_option_meta' ) ) {
    function sadesmarket_theme_option_meta( $meta_id, $option_key, $key_meta = '', $default = '' )
    {
        $ID = get_the_ID();
        ...
        if ( $option_key == null ) {
            ...
            $theme_options = sadesmarket_get_option( $option_key, $default );
        }
...
if ( !function_exists( 'sadesmarket_get_logo' ) ) {
    function sadesmarket_get_logo()
    {
        $tag         = is_home() ? 'h1' : 'div';
        $logo_link   = apply_filters( 'ovic_get_link_logo', home_url( '/' ) );
        $logo        = sadesmarket_theme_option_meta( '_custom_metabox_theme_options', 'logo' );
        $logo_mobile = sadesmarket_get_option( 'logo_mobile' );
        $text_1      = apply_filters( 'sadesmarket_logo_text_1', esc_html__( 'sades', 'sadesmarket' ) );
        $text_2      = apply_filters( 'sadesmarket_logo_text_2', esc_html__( 'market', 'sadesmarket' ) );
        if ( sadesmarket_is_mobile() && !empty( $logo_mobile ) ) {
            $logo = $logo_mobile;
        }
        $html = '<' . $tag . ' class="logo">';
        if ( $logo != '' ) {
        ...
        return apply_filters( 'sadesmarket_site_logo', $html );
    }
}
/**
 ...
 *
 */
if ( !function_exists( 'sadesmarket_do_shortcode' ) ) {
    function sadesmarket_do_shortcode( $tag, array $atts = array(), $content = null )
    {
...
if ( !function_exists( 'sadesmarket_locate_template' ) ) {
    function sadesmarket_locate_template( $template_name, $template_path = '', $default_path = '' )
    {
        if ( !$template_path ) {
        ...
        return apply_filters( 'sadesmarket_locate_template', $template, $template_name, $template_path );
    }
}
if ( !function_exists( 'sadesmarket_get_template' ) ) {
    function sadesmarket_get_template( $template_name, $args = array(), $template_path = '', $default_path = '' )
    {
        $cache_key = sanitize_key( implode( '-', array(
            'sadesmarket-template',
            $template_name,
            $template_path,
            $default_path,
            SADESMARKET
        ...
        if ( !$template ) {
            $template = sadesmarket_locate_template( $template_name, $template_path, $default_path );
            wp_cache_set( $cache_key, $template, 'ovic' );
        }

        // Allow 3rd party plugin filter template file from their plugin.
        $filter_template = apply_filters( 'sadesmarket_get_template', $template, $template_name, $args, $template_path, $default_path );
        ...
        if ( $filter_template !== $template ) {
            if ( !file_exists( $filter_template ) ) {
                ...
                _doing_it_wrong(
                    __FUNCTION__,
                    sprintf( __( '%s does not exist.', 'sadesmarket' ), '<code>' . $template . '</code>' ),
                    '2.1'
                );
        ...
        if ( !empty( $args ) && is_array( $args ) ) {
            if ( isset( $args['action_args'] ) ) {
                _doing_it_wrong(
                    __FUNCTION__,
                    __( 'action_args should not be overwritten when calling sadesmarket_get_template.', 'sadesmarket' ),
                    '3.6.0'
                );
        ...
        do_action( 'sadesmarket_before_template_part', $action_args['template_name'], $action_args['template_path'], $action_args['located'], $action_args['args'] );

        include $action_args['located'];

        do_action( 'sadesmarket_after_template_part', $action_args['template_name'], $action_args['template_path'], $action_args['located'], $action_args['args'] );
    }
}
/**
 *
 * QUERY POSTS
 **/
if ( !function_exists( 'sadesmarket_shortcode_posts_query' ) ) {
    function sadesmarket_shortcode_posts_query( $atts, $post_type = 'post' )
    {
        global $post;
 ...
 * QUERY PRODUCTS
 **/
if ( !function_exists( 'sadesmarket_shortcode_products_query' ) ) {
    function sadesmarket_shortcode_products_query( $atts, $exclude_id = false )
    {
 ...
 **/
if ( !function_exists( 'sadesmarket_resize_image' ) ) {
    function sadesmarket_resize_image( $attachment_id, $width, $height, $crop = false, $use_lazy = false, $placeholder = true, $class = '' )
    {
 ...
 * GET OPTIONS
 **/
if ( !function_exists( 'sadesmarket_file_options' ) ) {
    function sadesmarket_file_options( $path, $name, $is_block = false )
    {
        $cache_key = '';
        $id_block  = $is_block ? 'block' : 'none-block';
        if ( sadesmarket_get_option( 'enable_cache_option' ) ) {
            $cache_key = sanitize_key( implode( '-', array(
                'sadesmarket-file-options',
                $name,
                $id_block,
                $path,
                SADESMARKET
...
if ( !function_exists( 'sadesmarket_footer_preview' ) ) {
    function sadesmarket_footer_preview( $inherit = false )
    {
        $cache_key = '';
        if ( sadesmarket_get_option( 'enable_cache_option' ) ) {
            $cache_key = sanitize_key( implode( '-', array( 'sadesmarket-footer-preview', SADESMARKET ) ) );
            $options   = get_transient( $cache_key );
            if ( $options ) {
                if ( $inherit == false ) {
        ...
        $footer_preview['inherit'] = array(
            'title'   => esc_html__( 'Inherit Desktop', 'sadesmarket' ),
            'preview' => get_theme_file_uri( '/assets/images/placeholder.jpg' ),
        );
        $footer_preview['none']    = array(
            'title'   => esc_html__( 'None', 'sadesmarket' ),
            'preview' => get_theme_file_uri( '/assets/images/placeholder.jpg' ),
        );
        $args                      = array(
...
if ( !function_exists( 'sadesmarket_social_option' ) ) {
    function sadesmarket_social_option( $reverse = false )
    {
        $socials     = array();
        $all_socials = sadesmarket_get_option( 'user_all_social' );
...
if ( !function_exists( 'sadesmarket_product_options' ) ) {
    function sadesmarket_product_options( $allow = 'Theme Option', $is_block = false )
    {
        $cache_key = '';
        $id_block  = $is_block ? 'block' : 'none-block';
        if ( sadesmarket_get_option( 'enable_cache_option' ) ) {
            $cache_key = sanitize_key( implode( '-', array( 'sadesmarket-product-options', $allow, $id_block, SADESMARKET ) ) );
            $options   = get_transient( $cache_key );
            if ( $options ) {
                return $options;
            }
        ...
        if ( empty( $product_options ) ) {
            $product_options['no-product'] = array(
                'title' => esc_html__( 'No Product Found', 'sadesmarket' ),
            );
        }
...
if ( !function_exists( 'sadesmarket_page_layout' ) ) {
    function sadesmarket_page_layout()
    {
        if ( class_exists( 'WooCommerce' ) && is_woocommerce() ) {
            $sidebar_layout = sadesmarket_get_option( 'sidebar_shop_layout', 'left' );
            $sidebar_name   = sadesmarket_get_option( 'shop_used_sidebar', 'shop-widget-area' );
            if ( is_product() ) {
                $sidebar_layout = 'full';
                $sidebar_name   = '';
            ...
            $sidebar_name   = sadesmarket_theme_option_meta( '_custom_page_side_options', null, 'page_sidebar', 'widget-area' );
            $sidebar_layout = sadesmarket_theme_option_meta( '_custom_page_side_options', null, 'sidebar_page_layout', 'left' );
            if ( !is_active_sidebar( $sidebar_name ) ) {
                $sidebar_layout = 'full';
            }
        } else {
            $sidebar_layout = sadesmarket_get_option( 'sidebar_blog_layout', 'left' );
            $sidebar_name   = sadesmarket_get_option( 'blog_used_sidebar', 'widget-area' );
            if ( is_single() ) {
                $sidebar_layout = sadesmarket_get_option( 'sidebar_single_layout', 'right' );
                $sidebar_name   = sadesmarket_get_option( 'single_used_sidebar', 'widget-area' );
            }
            if ( !is_active_sidebar( $sidebar_name ) ) {
                $sidebar_layout = 'full';
            }
...
if ( !function_exists( 'sadesmarket_breadcrumb' ) ) {
    function sadesmarket_breadcrumb()
    {
        if ( class_exists( 'WooCommerce' ) ) {
...
if ( !function_exists( 'sadesmarket_page_title' ) ) {
    function sadesmarket_page_title()
    {
        if ( is_home() ) : ?>
            <?php if ( is_front_page() ) : ?>
                <h1 class="page-title entry-title">
                    <span><?php esc_html_e( 'Latest Posts', 'sadesmarket' ); ?></span>
                </h1>
            <?php else: ?>
                <h1 class="page-title entry-title">
        ...
        <?php else: ?>
            <?php if ( is_search() ) : ?>
                <h1 class="page-title entry-title">
                    <span><?php printf( esc_html__( 'Search Results for: "%s"', 'sadesmarket' ), '<span class="search-results">' . get_search_query() . '</span>' ); ?></span>
                </h1>
            <?php else: ?>
 ...
 * );
 */
if ( !function_exists( 'sadesmarket_add_action' ) ) {
    function sadesmarket_add_action( $functions, $reverse = false )
    {
        if ( !empty( $functions ) ) {
...
if ( !function_exists( 'sadesmarket_get_form_newsletter' ) ) {
    function sadesmarket_get_form_newsletter()
    {
        $cache_key = '';
        if ( sadesmarket_get_option( 'enable_cache_option' ) ) {
            $cache_key = sanitize_key( implode( '-', array( 'sadesmarket-form-newsletter', SADESMARKET ) ) );
            $options   = get_transient( $cache_key );
            if ( $options ) {
                return $options;
            }
        ...
        $list_form[ $default_id ] = esc_html__( 'Default Form', 'sadesmarket' );
...
if ( !function_exists( 'sadesmarket_mobile_menu' ) ) {
    function sadesmarket_mobile_menu( $menu_locations, $default = 'primary' )
    {
        if ( !empty( $menu_locations ) ) {
            ...
            $mobile_menu .= "<span class='ovic-menu-current-panel-title'>" . esc_html__( 'Main Menu', 'sadesmarket' ) . "</span>";
